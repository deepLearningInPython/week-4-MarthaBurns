...
punct = "!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~"
tokens = ["".join(char for char in word if char not in punct) for word in text.split()]
...
def token_counts(string: str, k: int = 1) -> dict:
    ...
    word_frequencies = {word: words.count(word) for word in set(words) if words.count(word) > k}
...
def make_vocabulary_map(documents: list) -> tuple:
    tokenized = [tokenize(stuff) for stuff in documents]
    all_tokens = [token for doc in tokenized for token in doc]
    u_words = np.unique(all_tokens)
    token_to_id = {word: i for i, word in enumerate(u_words)}
    id_to_token = {i: word for word, i in token_to_id.items()}
...
def tokenize_and_encode(documents: list) -> list:
    ...
    encoded_sentences = [[token_to_id[token] for token in tokenize(doc)] for doc in documents]
